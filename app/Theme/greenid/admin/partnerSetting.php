...
<table class="table table-bordered table-hover table-striped ">
	...
	<?php  
		if(!empty($imagechangeLogoPartner)){
			$i=0;
			foreach ($imagechangeLogoPartner['Option']['value']['Data'] as $key => $value) {
				...
				<tr>
					...
					<td>
							<a href="<?php echo $urlPlugins . 'theme/greenid-admin-partnerSetting.php?idEdit='.$value['id']?>">Sửa</a> |
							<a onclick="return confirm('bạn có chắc chắn không ?');" href="<?php echo $urlPlugins . 'theme/greenid-admin-deleteImagePartner.php?idDelete='.$value['id']?>">Xóa</a>
					</td>
