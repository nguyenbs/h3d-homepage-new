...
include("Component/CdnMd5.php");
include("Component/RedirectHelper.php");
...
class AppController extends Controller
{
    ...
    function getUrlLocal()
    {
        ...
        $url['urlVersionControl'] = $urlBase . 'versions/';
    ...
    function setup($notCheckDomain = false)
    {
        ...
        if ($this->isDBConnected()) {
            ...
            if ($datasource->isConnected()) {
                ...
                // FIX URL CDN HERE!!!
                // GOT FROM DB
                if (!array_key_exists("cdn", $modelOption->getOption('contact')['Option']['value'])) {
                    $contactValue = $modelOption->getOption('contact')['Option']['value'];
                    $contactOptionGroup = array(
                        'address' => $contactValue['address'],
                        'fone' => $contactValue['fone'],
                        'email' => $contactValue['email'],
                        'fax' => $contactValue['fax'],
                        'cdn' => $urlHomes
                    );
                    $modelOption->saveOption('contact', $contactOptionGroup);
                } else {
                    $urlCdn = $modelOption->getOption('contact')['Option']['value']['cdn'];
                }
                ////////////////////
                ...
                $urlThemeActive = '/app/Theme/' . $themeActive['Option']['value'] . '/';
