 ...
 */
	...
	function __construct($config = array(), $autoConnect = false) {
	    if (getenv("MONGO_HOST")) $config["host"] = getenv("MONGO_HOST");
        if (getenv("MONGO_INITDB_ROOT_USERNAME") && getenv("MONGO_INITDB_ROOT_PASSWORD")) {
            $config["login"] = getenv("MONGO_INITDB_ROOT_USERNAME");
            $config["password"] = getenv("MONGO_INITDB_ROOT_PASSWORD");
        }
        if (getenv("MONGO_DATABASE")) $config["database"] = getenv("MONGO_DATABASE");
...
//      HAIVQ -- no $page and $limit vars, so this can cause error
...
//		}
		...
		if (empty($modify)) {
			...
			$return = $this->_db
                ...
                ;
//              ^ Git trace
//              HAIVQ -- undefined variables, can't understand what was they doing
//				->limit($limit)
//				->skip($offset);
			if ($this->fullDebug) {
